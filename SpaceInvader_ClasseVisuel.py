...
Fait: Initialisation de la fenetre avec tous les parametres actuellement utiles;
    methodes des boutons, labels et menus;
    methodes standard propre au Canvas GameZone;
    debut des methodes avances propres au Canvas GameZone;
    methode d'ecoute du clavier fAction;
    toutes les images sont importees;
...
class Visuel():
    ...
    def __init__(self, mg):
        ...
        self.buttonQuit = Button(self.mv, text = "QUITTER", command = self.mv.destroy, bg = "#B0E0E6")
        self.buttonNewGame = Button(self.mv, text = "NEW GAME", command = self.fNewGame, bg = "#B0E0E6")
        ...
        #Creation des apparences des entites
        ...
        #Creation d'une liste des entites affichees
        self.entityId = []

        #Apparition du joueur sur le Canvas GameZone
        self.fAffichage(self.mg.joueur)

        #Apparition des blocs de protection
    ...
    def fCreate_regle(self):
        ...
        Label(TitreRegle, text = "Tirez sur les Sucreries afin de les faire disparaître avant que celles-ci n'atteignent votre Dent. Protégez-vous à l'aide des tablettes de chocolat !").pack()
        Label(TitreRegle, text = "Votre dent possède trois vies et en perd une à chaque contact avec une sucrerie ennemie. De plus, l'élimination d'une sucrerie classique vous octroiera").pack()
        Label(TitreRegle, text = "100 points, celle d'une sucrerie spéciale le DOUBLE. Mais faites attention les sucreries n'en finissent pas d'apparaître et elles deviennent de plus").pack()
        Label(TitreRegle, text = "en plus savoureuses ! A chaque vague détruite le nombre de vies des sucreries classiques augmente, à vous de voir jusqu'où vous pourrez protéger votre dent.").pack()
        Commande = LabelFrame(regle, text = "Commandes", padx = 20, pady = 20)
        Commande.pack(fill = 'both', expand = 'yes')
        Label(Commande, text = "- utiliser les flèches directionnelles pour vous déplacer (le joueur ne se déplace que vers la gauche ou vers la droite)").pack()
        Label(Commande, text = "- utiliser la barre espace pour tirer").pack()
        Label(Commande, text = "- utiliser la touche entrée (Return pour tkinter) pour lancer le jeu ou le mettre").pack()
    ...
    def fNewGame(self):
        '''Associe au bouton NEWGAME, met la valeur True au parametre NewGame.'''
    ...
    def fScore(self):
        ...
        self.valeurscore = self.mg.Totpts                               #le score est egal au total des points du jeu
    ...
    def fResetVisuel(self):
        ...
        #Si la ligne suivante est decommentee la vitesse du jeu va augmenter a chaque NewGame
        #self.fGestionTour()
    
    #Methodes standards propres au Canvas GameZone
    def fAffichage(self, objet):
        '''Affiche l'objet sur le Canvas GameZone'''
        #Test le type de l'entite pour savoir quelle image y associer
        if objet.type == 1:
        ...
        #Lie le parametre id de l'entite a son image a l'ecran puis l'ajoute dans la liste contenant les id des entites a l'ecran
    ...
    def fDeplace(self, objet, direction : int, sens : int):
        ...
        if direction == 0:
            ...
            #Test pour savoir si l'entite sort de la zone de jeu, soit (10, 602) pour les x et y
            #Si l'entite sort de la zone de jeu, effectuer un mouvement pour la remettre dans la zone de jeu et mettre sa position aux limites de la zone
        ...
        #Gere les deplacements sur l'axe des y, meme principe que pour l'axe des x
            ...
            #Si l'entite est un tir et qu'elle sort des limites, alors lui retirer une vie. C'est equivalent a la detruire
    ...
    #Methodes avances propres au Canvas GameZone
    def fMoveEnnemi(self):
        ...
        if self.mg.ennemidir == 1:
            #Deplcae tous les ennemis classiques dans ce sens
    ...
    def fMoveTir(self):
        ...
        #Parcourt la liste des tirs et les deplace tous, le sens est en positif car la vitesse des tirs est mise en négative pour ceux du joueur et positive pour ceux des ennemis
    ...
    def fAffichageBlocs(self):
        '''Affiche les blocs de protection au debut du jeu'''
    ...
    def fAffichageEnnemiSpeciaux(self):
        ...
        if self.mg.ennemisspeciaux == [] and randint(0,100) == 42:          #Test s'il n'y a pas d'ennemis speciaux en jeu et si une condition random est respectee
    ...
    def fActionJoueur(self, event):
        ...
        if action == 'Left' and self.mg.run == True:
            ...
            #self.mg.fTirPossible()                                                             Tentative infructueuse pour introduire un delai entre les tirs du joueur
            self.mg.fCreation(-1, 1, 0, [self.mg.joueur.position[0] + 13, self.mg.joueur.position[1] - 6], (5, 5), [0, -10])
            self.fAffichage(self.mg.tirs[-1])
            #self.mv.after(2000, self.mg.fTirPossible())                                        Tentative infructueuse pour introduire un delai entre les tirs du joueur
        if action == 'Return':
            if self.mg.run == True:
                self.mg.run = False
                print("PAUSE")
    ...
    def fGestionTour(self):
        ...
        if self.mg.gameover == False:                   #Regarde si le joueur n'a pas perdu
            if self.mg.run == True:                     #Regarde si le jeu n'est pas en pause
                self.fAffichageEnnemiSpeciaux()         #Regarde si un ennemi special peut etre cree et si les conditions sont reunies le cree
                self.mg.fTrouverRepere()                #Trouve l'ennemi classique de repere
                self.fMoveEnnemiSpeciaux()              #Deplace les ennemis speciaux
                self.fMoveEnnemi()                      #Deplace les ennemis classiques grace a l'ennemi de repere
                self.fMoveTir()                         #Deplace les tirs
                self.mg.fCollision(self.mg.joueur)      #Verifie si le joueur entre en collision avec une entite   
                for entity in self.mg.entity:           #Verifie les collisions de toutes les autres entites
                        self.mg.fCollision(entity)
                        self.fSupprimer(entity)         #Supprime les entites ennemies n'ayant plus de vie
                ...
                if self.mg.ennemis == []:               #Si la vague actuelle d'ennemis est detruite en fabrique une nouvelle
